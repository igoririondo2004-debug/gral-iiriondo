...
# CONFIG
# =====================================================
MODEL_NAME = "ministral-3:3b"   # 👈 change to your exact Ollama tag if different
...
# =====================================================
# DEVICE CHECK (REALITY FIXED)
# =====================================================
def check_device():
    try:
        # This only shows GPU activity indirectly
        nvidia = subprocess.check_output(["nvidia-smi"], text=True).lower()

        if "ollama" in nvidia or "python" in nvidia:
            device = "GPU (active)"
        else:
            device = "GPU available (status unknown)"
