...
# =====================================================
# DEVICE CHECK (REALISTIC FOR OLLAMA)
# =====================================================
def check_device():
    try:
        out = subprocess.check_output(["ollama", "ps"], text=True).lower()

        if "gpu" in out or "cuda" in out:
            device = "GPU (Ollama active)"
        else:
            device = "CPU (or unknown via Ollama)"

        print(f"🧠 Backend detected: {device}")
        return device

    except Exception:
        print("🧠 Backend detected: UNKNOWN (cannot query ollama ps)")
        return "unknown"
